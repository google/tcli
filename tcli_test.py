...
__package__ = 'tcli'  # pylint: disable=redefined-builtin
...
terminal_size = tcli.terminal.TerminalSize


class UnitTestGlobalDefs(unittest.TestCase):
  """Tests the top level functions."""

  def setUp(self):
    super(UnitTestGlobalDefs, self).setUp()
...
class UnitTestTCLI(unittest.TestCase):
  ...
  def tearDown(self):
    ...
    tcli.terminal.TerminalSize = terminal_size
  ...
  def testAdvancedRegexpTargets(self):
    ...
    self.assertEqual(
        ('somecommand', ['^br\d{2}.*'], False),
        self.tcli_obj.cli_parser.ParseCommandLine("somecommand '^br\d{2}.*'"))

    # Complex tilde commands targets/xtargets must be quoted.
    # pylint: disable=anomalous-backslash-in-string
    self.assertEqual(
        ('somecommand', ['^brd{2}.*'], False),
        self.tcli_obj.cli_parser.ParseCommandLine('somecommand ^br\d{2}.*'))
